import { c as createComponent, r as renderTemplate, g as addAttribute, h as createAstro, i as renderComponent, m as maybeRenderHead, j as renderSlot, k as renderHead, l as Fragment, u as unescapeHTML, n as defineStyleVars } from './astro_DjHzFfOT.mjs';
import 'kleur/colors';
/* empty css                         */
import 'clsx';
...
import { $ as $$Image } from './pages/generic_05OKl069.mjs';
...
async function getAllUris() {
  ...
  const uris = Object.values(data).reduce((acc, currentValue) => acc.concat(currentValue.nodes), []).filter((node) => node.uri !== null).map((node) => {
    if (node.uri === "/") {
      return { params: { uri: "/" } };
    }
...
const $$ImageBlock = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="image-block"> ${renderComponent($$result, "Image", $$Image, { "src": sourceUrl, "width": "10", ";": true, "height": "10", ";": true, "widths": [240, 540, 720], "sizes": `(max-width: 360px) 240px, (max-width: 720px) 540px, (max-width: 1600px) 720px`, "format": "afiv", "alt": altText || "" })} </div>`;
...
const $$VideoBlock = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div${addAttribute(["custom-video", size], "class:list")}> <video preload="metadata" tabindex="-1" disablepictureinpicture playsinline loop title=""${addAttribute(url, "src")}${addAttribute(true, "autoplay")} muted></video> </div>`;
